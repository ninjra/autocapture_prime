...
DEFAULT_THUMB_PROMPT = (
    "You are a strict UI detector. Return JSON only.\n"
    "Schema:\n"
    ...
    '"windows":[{"label":"string","app":"string","context":"host|vdi|unknown","bbox_norm":[x1,y1,x2,y2],'
    '"visibility":"fully_visible|partially_occluded|unknown","z_hint":0.0}]}\n'
    "Rules: bbox_norm must be normalized [0,1] and valid; labels/apps must be concrete values from the image. "
    "Do not emit placeholder literals like 'string' or enum lists. Return only JSON."
)

DEFAULT_ROI_PROMPT = (
    "You are a strict UI parser for one cropped region. Return JSON only.\n"
    "Schema:\n"
    '{"elements":[{"type":"window|pane|tab|button|row|cell|text|icon|list_item|other",'
    '"label":"string","bbox_norm":[x1,y1,x2,y2],"state":{"enabled":true,"selected":false,"focused":false,"expanded":false},'
    '"interactable":true}],'
    '"windows":[{"label":"string","app":"string","context":"host|vdi|unknown","bbox_norm":[x1,y1,x2,y2],'
    '"visibility":"fully_visible|partially_occluded|unknown","z_hint":0.0}],'
    '"facts":[{"key":"string","value":"string","confidence":0.0}]}\n'
    "Rules: bbox_norm must be normalized [0,1] and valid; labels/text must come from the crop. "
    "Do not emit placeholder literals like 'string' or enum lists. Return only JSON."
)
...
class VllmVLM(PluginBase):
    def __init__(self, plugin_id: str, context: PluginContext) -> None:
        ...
        self._max_rois = max(1, int(cfg.get("max_rois") or 4))
        self._roi_max_side = max(512, int(cfg.get("roi_max_side") or 896))
        self._thumb_max_tokens = max(128, int(cfg.get("thumb_max_tokens") or 320))
    ...
    def extract(self, image_bytes: bytes) -> dict[str, Any]:
        ...
        if self._two_pass_enabled and _PIL_AVAILABLE:
            ...
            if _valid_layout(layout) and len(elements) >= 2:
    ...
    def _run_two_pass(self, client: OpenAICompatClient, image_bytes: bytes) -> dict[str, Any]:
        ...
        out["layout"] = layout
        out["text"] = json.dumps({"elements": layout_elements}, sort_keys=True, separators=(",", ":"))
        out["model_id"] = self._model
