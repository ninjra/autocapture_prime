...
class _System:
    def __init__(self, config, metadata, media, vlm, events):
        self.config = config
        self._caps = {
...
class IdleGpuConcurrencyTests(unittest.TestCase):
    ...
    def test_gpu_concurrency_zero_skips_vlm(self) -> None:
        config = {
            "runtime": {"run_id": "run1"},
            "processing": {
                "idle": {
        ...
        metadata.put(record_id, self._base_record())
        media = _MediaStore({record_id: _zip_blob()})
        system = _System(config, metadata, media, _Extractor("vlm text"), _EventBuilder())
        processor = IdleProcessor(system)

        _done, stats = processor.process_step(budget_ms=0)

        self.assertEqual(stats.processed, 0)
        derived = [k for k in metadata.data.keys() if k.startswith("run1/derived.text.vlm/")]
        self.assertEqual(derived, [])

    def test_gpu_concurrency_allows_vlm(self) -> None:
        config = {
            "runtime": {"run_id": "run1"},
            "processing": {
                "idle": {
