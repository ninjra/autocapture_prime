...
class CaptureDropEventTests(unittest.TestCase):
    def test_drop_event_emitted_when_queue_full(self) -> None:
        frames = [
            Frame(ts_utc="t0", data=b"x", width=1, height=1, ts_monotonic=0.0),
            Frame(ts_utc="t1", data=b"y", width=1, height=1, ts_monotonic=0.1),
            Frame(ts_utc="t2", data=b"z", width=1, height=1, ts_monotonic=0.2),
        ]
        config = {
            ...
            "backpressure": {"max_fps": 30, "max_bitrate_kbps": 8000, "max_queue_depth": 1},
        ...
        pipeline._frame_queue = BoundedQueue(1, "drop_oldest")

        with patch("autocapture_nx.capture.pipeline._frame_iter", return_value=("mss", iter(frames))):
            with patch("autocapture_nx.capture.pipeline.time.monotonic", return_value=0.0):
                pipeline._grab_loop()

        self.assertIn("capture.drop", builder.events)
        self.assertIn("capture.drop", builder.ledger_events)
