...
class FixtureQueryCoverageTests(unittest.TestCase):
    def setUp(self) -> None:
        try:
    ...
    def test_auto_query_coverage(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            frame_dir = tmp_path / "frames"
            frame_dir.mkdir(parents=True, exist_ok=True)
            png_path = frame_dir / "fixture.png"
            self._write_png(png_path, "ALPHA BETA GAMMA DELTA")

            manifest = {
                "fixture_id": "test-auto",
                "version": 1,
            ...
            self.assertGreaterEqual(int(queries.get("count", 0)), 4)
