...
def test_gate_golden_pipeline_triplet_rejects_synthetic_q40_source_tier(tmp_path: pathlib.Path) -> None:
    popup = tmp_path / "popup.json"
    q40 = tmp_path / "q40.json"
    temporal = tmp_path / "temporal.json"
    out = tmp_path / "out.json"
    ...
    payload = json.loads(out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert "q40.source_tier_not_real" in payload["failure_reasons"]
