...
def _matrix_signature(matrix: dict[str, Any]) -> str:
    core = {
        ...
        "strict_mode": bool(matrix.get("strict_mode", False)),
        ...
        "advanced_failed_ids": list((matrix.get("advanced20", {}) or {}).get("failed_ids", []) or []),
        "generic_failed_ids": list((matrix.get("generic20_contract", {}) or {}).get("failed_ids", []) or []),
    }
    return _canonical_hash(core)


def _strict_semantics_ok(matrix: dict[str, Any]) -> tuple[bool, list[str]]:
    reasons: list[str] = []
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run strict Q40 synthetic gauntlet N times and fail on drift.")
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Artifact mode: evaluate determinism from provided matrix JSON files (repeat flag).",
    )
    parser.add_argument("--runs", type=int, default=5)
    parser.add_argument("--out", default="artifacts/advanced10/q40_determinism_gate_latest.json")
    ...
    if matrix_paths:
        ...
        for idx, path in enumerate(matrix_paths):
            ...
            strict_ok, strict_reasons = _strict_semantics_ok(matrix_obj)
