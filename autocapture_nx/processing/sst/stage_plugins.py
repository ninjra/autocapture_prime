    ...
    def run(self, inp: PluginInput, ctx: RunContext) -> PluginOutput:
        ...
        if isinstance(existing_graph, dict):
            ...
            if (
                ...
                return PluginOutput(
                    items={"element_graph": existing_graph, "element_graph_raw": existing_graph},
                    metrics={"ui.elements": float(len(existing_elements))},
                    diagnostics=diagnostics,
                )
    ...
    *,
    ...
    for provider_id, provider in providers:
        ...
        if backend in non_recoverable_backends:
            diagnostics.append({"kind": "sst.ui_vlm_backend_unavailable", "provider_id": provider_id, "backend": backend})
            continue
        ...
        if element_graph is None and isinstance(source, str):
            recovered_layout = _recover_layout_from_partial_json(source, frame_bbox)
            if recovered_layout:
                state_id = "vlm"
                ...
                if element_graph is not None:
                    diagnostics.append(
                        {
                            "kind": "sst.ui_vlm_recovered_partial_json",
                            "provider_id": provider_id,
                            "backend": backend,
                            "elements": len(cast(list[Any], recovered_layout.get("elements", []))),
                        }
                    )
        if element_graph:
            if len(element_graph.get("elements", ())) <= 1:
                diagnostics.append({"kind": "sst.ui_vlm_graph_too_small", "provider_id": provider_id, "backend": backend})
                continue
            successful += 1
            ...
            # Early exit once we have a strong VLM-grounded graph from a non-heuristic backend.
            if best_score >= 180:
                break
            # Respect max_providers as a success cap, but keep searching if quality is weak.
...
def _parse_element_graph_from_cached_vlm_tokens(
    *,
    ...
    for token in tokens:
        ...
        if element_graph is None:
            recovered_layout = _recover_layout_from_partial_json(candidate_text, frame_bbox)
            if recovered_layout:
                element_graph = _parse_element_graph(
                    recovered_layout,
                    tokens,
...
def _providers(capability: Any) -> list[tuple[str, Any]]:
    ranked = capability_providers(capability, "vision.extractor")
    ranked.sort(key=lambda pair: (-_provider_priority(pair[0]), str(pair[0])))
    return ranked
...
def _recover_layout_from_partial_json(text: str, frame_bbox: tuple[int, int, int, int]) -> dict[str, Any]:
    blob = str(text or "")
    if not blob:
        return {}
    elements: list[dict[str, Any]] = []
...
def _coerce_bbox(bbox: Any, frame_bbox: tuple[int, int, int, int]) -> tuple[int, int, int, int] | None:
    ...
    if width <= 0 or height <= 0:
        return None
    # Accept both absolute pixels and normalized [0,1] coordinates from VLM backends.
    if (
        0.0 <= x1f <= 1.0
        and 0.0 <= y1f <= 1.0
        and 0.0 <= x2f <= 1.0
