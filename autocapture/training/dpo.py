"""DPO training pipeline (deterministic manifest)."""
...
def run_dpo(
    ...
    *,
    ...
    payload = {"dataset_hash": dataset.dataset_hash, "params": params, "method": "dpo"}
    artifact_data = json.dumps(payload, sort_keys=True).encode("utf-8")
    artifact_path = output_dir / "dpo_weights.bin"
    if not dry_run:
